clear
clc

addpath('D:/casadi/');
import casadi.*

parameter = [1.3,1.6;1000,850];
TauMax = 40000;
% Degree of interpolating polynomial
d = 3;

% Get collocation points
tau = collocation_points(d, 'legendre');

% Collocation linear maps
[C,D,B] = collocation_coeff(tau);

% Time horizon
T = 10;

% Declare model variables
q1 = SX.sym('q1');
q2 = SX.sym('q2');
dq1 = SX.sym('dq1');
dq2 = SX.sym('dq2');
dq = [dq1;dq2];
q = [q1; q2];
x = [q;dq];
u1 = SX.sym('u1');
u2 = SX.sym('u2');
u = [u1;u2];
pinvB = pinv(B_f(parameter,q));

% Model equations
ddq = pinvB*(u - G_f(parameter,q) - C_f(parameter,q,dq)*dq);
xdot = [dq1;dq2;ddq];

% Objective term
L = u2^2 + u1^2;

% Continuous time dynamics
f = Function('f', {x, u}, {xdot, L});

N = 250; % number of control intervals
h = T/N;

opti = Opti();
J = 0;

% "Lift" initial conditions
Xk = opti.variable(4);
% opti.subject_to(-pi < Xk(1) < pi);
% opti.subject_to(-pi < Xk(2) < pi);
%parte fermo in  q1 = 0 q2 = 0
opti.set_initial(Xk, [0;0;0;0]);  

% Collect all states/controls
Xs = {Xk};
Us = {};

% Formulate the NLP
for k=0:N-1
   % New NLP variable for the control
   Uk = opti.variable();
   Us{end+1} = Uk;
   opti.subject_to(-TauMax <= Uk <= TauMax);
   opti.set_initial(Uk, 0);

   % Decision variables for helper states at each collocation point
   Xc = opti.variable(4, d);
   
   % Evaluate ODE right-hand-side at all helper states
   [ode, quad] = f(Xc, Uk);

   % Add contribution to quadrature function
   J = J + quad*B*h;

   % Get interpolating points of collocation polynomial
   Z = [Xk Xc];

   % Get slope of interpolating polynomial (normalized)
   Pidot = Z*C;
   % Match with ODE right-hand-side 
   opti.subject_to(Pidot == h*ode);

   % State at end of collocation interval
   Xk_end = Z*D;

%    % New decision variable for state at end of interval
%    Xk = opti.variable(4);
%    Xs{end+1} = Xk;
   opti.subject_to(0.99 < Xk(1) < 1.04);
   opti.subject_to(-1.08 < Xk(2) < -1.03);
% %    opti.set_initial(Xk, [0;0;0;0]);
% 
   % Continuity constraints
   opti.subject_to(Xk_end==Xk)
end

Xs = [Xs{:}];
Us = [Us{:}];

opti.minimize(J);

p_opts = struct('expand',true);
s_opts = struct('max_iter',1000);
opti.solver('ipopt',p_opts,s_opts);

sol = opti.solve();

x_opt = sol.value(Xs);
u_opt = sol.value(Us);

x_opt = sol.value(Xs);
u_opt = sol.value(Us);

% Plot the solution
tgrid = linspace(0, T, N+1);
clf;
hold on
plot(tgrid, x_opt(1,:), '--')
plot(tgrid, x_opt(2,:), '-')
stairs(tgrid, [u_opt nan], '-.')
xlabel('t')
legend('x1','x2','u')